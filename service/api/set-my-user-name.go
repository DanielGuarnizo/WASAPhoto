...
func (rt *_router) setMyUserName(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
	...
	// Decode the request body to get the updated profile data
	var UpdatedProfile Profile 
	err:= json.NewDecoder(r.Body).Decode(&UpdatedProfile)

	if err != nil {
		// Error decoding request body, return 400 Bad Request
		w.WriteHeader(StatusBadRequest)
		return 
	}

	// Apply the partial update to the user profile
	profiles[userIndex].Username = updatedProfile.Username

	// Respond with the updated user profile
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(http.StatusOK)
	err = json.NewEncoder(w).Encode(profiles[userIndex])
	if err != nil {
